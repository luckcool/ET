    ...
    public class ComponentInfo
    {
        ...
        public void SetVariableInfoTypeName()
        {
            for (int index = 0; index < this.VariableInfos.Count; index++)
            {
                ...
                {
                    if (FUICodeSpawner.PackageInfos.TryGetValue(this.PackageId, out PackageInfo sourcePackageInfo))
                    {
                        if (FUICodeSpawner.PackageInfos.TryGetValue(packageId, out PackageInfo targetPackageInfo))
                        {
                            if (sourcePackageInfo.PackageComponentInfos.TryGetValue(this.Name, out PackageComponentInfo packageComponentInfo))
                            {
                                if (!packageComponentInfo.UsedPackages.Contains(targetPackageInfo.Name))
                                {
                                    packageComponentInfo.UsedPackages.Add(targetPackageInfo.Name);
                                }
                            }
                        }
                    }
