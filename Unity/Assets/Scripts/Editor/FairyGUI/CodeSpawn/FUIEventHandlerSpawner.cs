...
namespace FUIEditor
{
    public static class FUIEventHandlerSpawner
    {
        public static void SpawnEventHandler(string packageName, ComponentInfo componentInfo)
        {
            ...
            sb.AppendLine($"using ET.Client.{packageName};");
            ...
            sb.AppendFormat("\t\tpublic void OnCreatePanel(FUIEntity fuiEntity)");
            sb.AppendLine();
            sb.AppendLine("\t\t{");
            sb.AppendLine($"\t\t\tfuiEntity.GComponent = FUI_{panelName}.CreateInstance();");
            sb.AppendLine("\t\t}");

            sb.AppendFormat("\t\tpublic void OnInitPanelCoreData(FUIEntity fuiEntity)");
            sb.AppendLine();
            sb.AppendLine("\t\t{");
            sb.AppendLine("\t\t\tfuiEntity.PanelCoreData.PanelType = UIPanelType.Panel;");
            sb.AppendLine("\t\t}");
            ...
            sb.AppendLine();
            sb.AppendFormat("\t\tpublic void OnTranslate(FUIEntity fuiEntity)");
            sb.AppendLine();
            sb.AppendLine("\t\t{");
            sb.AppendFormat("\t\t\tfuiEntity.GetComponent<{0}>().OnTranslate();", panelName);
            sb.AppendLine();
            sb.AppendLine("\t\t}");
