...
namespace FUIEditor
{
    public static class FUIPanelSystemSpawner
    {
        public static void SpawnPanelSystem(string packageName, ComponentInfo componentInfo, VariableInfo variableInfo = null)
        {
            ...
            sb.AppendLine($"\t[EntitySystemOf(typeof({panelName}))]");
            sb.AppendFormat("\tpublic static partial class {0}System", panelName);
            sb.AppendLine();
            sb.AppendLine("\t{");
            ...
            sb.AppendLine();
            sb.AppendFormat("\t\tpublic static void OnTranslate(this {0} self)", panelName);
            sb.AppendLine();
            sb.AppendLine("\t\t{");
            subComNames.ForEach(comName =>
            {
                sb.AppendLine($"\t\t\tself.{comName}.OnTranslate();");
            });
            sb.AppendLine("\t\t}");
